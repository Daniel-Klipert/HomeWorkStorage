...
public static class LoginValidationException extends Exception{}
    ...
    public static void validateLogin(String login) throws LoginValidationException {
        ...
        boolean lenght = login.length() < 20;
        if (!lenght)
            throw new LoginValidationException();
        for (int i = 0; i < login.length(); i++) {
            if (Pattern.matches("\\d", String.valueOf(login.charAt(i)))) {
                hasNum = true;
                continue;
            } else if (Pattern.matches("[A-Z]", String.valueOf(login.charAt(i)))) {
                hasBig = true;
                continue;
            } else if (Pattern.matches("[a-z]", String.valueOf(login.charAt(i)))) {
                hasSmall = true;
                continue;
            } else if (Pattern.matches("[_]", String.valueOf(login.charAt(i)))) {
                hasUnder = true;
                continue;
            } else {
                throw new LoginValidationException();
        ...
        if (!hasNum || !hasBig || !hasSmall || !hasUnder) {
            throw new LoginValidationException();
        }
    }
