...
public class HomeWork4 {
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for (int i = 0; i < stringList.size(); i++){
            ...
            is = true;
            for (char t : stringList.get(i).replace(" ", "").toCharArray()){
                if (Character.isLowerCase(t)){} else is = false;
            }
            if (is == true) corrected.add(stringList.get(i));
        }
        return corrected;
    }
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        ...
        for (String key : shopCart.keySet()) {
            ...
            for (String wrongKey : keys){
                shopCart.remove(wrongKey);
            }
